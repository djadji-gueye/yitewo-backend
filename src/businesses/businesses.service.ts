...
export class BusinessesService {
  ...
  async create(data: CreateBusinessListingDto) {
    ...
    const externalId = data.externalSourceId || null;
    if (externalId) {
      const existing = await this.prisma.businessListing.findUnique({ where: { externalSourceId: externalId } });
      if (existing) return existing;
    }
    if (data.googlePlaceId) {
      const existing = await this.prisma.businessListing.findUnique({ where: { googlePlaceId: data.googlePlaceId } });
      if (existing) return existing;
    }

    const baseSlug = this.slugify(`${name}-${city}`);
    let slug = data.slug ? this.slugify(data.slug) : baseSlug;
    for (let index = 1; await this.prisma.businessListing.findUnique({ where: { slug } }); index += 1) {
      slug = `${baseSlug}-${index}`;
    }
    ...
    return this.prisma.businessListing.create({
      data: {
        name, city, slug, category: this.normalizeText(data.category) || null,
        zone: this.normalizeText(data.zone) || null, address: this.normalizeText(data.address) || null,
        phone: this.normalizeText(data.phone) || null, email: this.normalizeText(data.email) || null,
        website: this.normalizeText(data.website) || null, description: this.normalizeText(data.description) || null,
        imageUrl: this.normalizeText(data.imageUrl) || null, latitude: data.latitude, longitude: data.longitude,
        googlePlaceId: data.googlePlaceId || null, externalSourceId: externalId,
        source: data.source ?? BusinessListingSource.MANUAL, status, isVisible: this.isVisibleStatus(status),
      },
    });
  }
  ...
  async importFromOsm(dto: ImportOsmBusinessDto) {
    ...
    if (!areaResponse.ok) throw new BadRequestException('Le service OpenStreetMap est indisponible.');
    const areas: any[] = await areaResponse.json();
    const area = areas[0];
    if (!area?.lat || !area?.lon) throw new BadRequestException(`Ville introuvable dans OpenStreetMap: ${city}.`);

    const tagFilter = category ? `nwr["${category === 'restaurant' ? 'amenity' : 'shop'}"="${category}"](around:15000,${area.lat},${area.lon});` :
    ...
    const overpassResponse = await fetch('https://overpass-api.de/api/interpreter', {
      method: 'POST', headers: { 'Content-Type': 'text/plain', 'User-Agent': 'Yitewo/1.0 contact@yitewo.com' }, body: query,
    });
    if (!overpassResponse.ok) throw new BadRequestException('Le service Overpass est temporairement indisponible.');
    const overpass: any = await overpassResponse.json();
    const elements = Array.isArray(overpass.elements) ? overpass.elements.slice(0, limit) : [];
    ...
    for (const element of elements) {
      ...
      const existing = await this.prisma.businessListing.findUnique({ where: { externalSourceId } });
      ...
      try {
        await this.create({
          ...
          zone: tags['addr:suburb'] || tags['addr:quarter'] || undefined, phone: tags.phone || tags['contact:phone'],
          website: tags.website || tags['contact:website'], latitude: lat, longitude: lon,
          source: BusinessListingSource.OPENSTREETMAP, status: BusinessListingStatus.VISIBLE, externalSourceId,
        });
        imported++;
      } catch { skipped++; }
    }
    return { city, requested: limit, imported, skipped, source: 'OPENSTREETMAP' };
  }

  async importFromGoogle(_dto: ImportGoogleBusinessDto) {
    throw new BadRequestException('L’import Google est désactivé. Utilisez /businesses/import/osm, sans carte bancaire.');
  }
  ...
  async findAll(filters?: { city?: string; category?: string; status?: string }) {
    ...
    if (filters?.status && Object.values(BusinessListingStatus).includes(filters.status as BusinessListingStatus)) where.status = filters.status as BusinessListingStatus;
    return this.prisma.businessListing.findMany({ where, orderBy: { createdAt: 'desc' } });
  }
  findMap() { return this.prisma.businessListing.findMany({ where: { isVisible: true, latitude: { not: null }, longitude: { not: null } }, orderBy: { createdAt: 'desc' } }); }
  async search(params?: { q?: string; city?: string; category?: string }) {
    ...
    if (city) where.city = { contains: city, mode: 'insensitive' }; if (category) where.category = { contains: category, mode: 'insensitive' };
  ...
  async findBySlug(slug: string) { const item = await this.prisma.businessListing.findUnique({ where: { slug } }); if (!item) throw new NotFoundException('Commerce introuvable.'); return item; }
  async claim(id: string) { await this.findById(id); return this.prisma.businessListing.update({ where: { id }, data: { isClaimed: true, claimRequestedAt: new Date(), status: BusinessListingStatus.PENDING } }); }
